...
const characters = [
  ...
  { 
    ...
    weakness: "Naive, gets easily hungry, sleeps less, and loses 10 gold each time he falls asleep"
  },
  { 
    name: "SAM",
    id: "sam",
  ...
  { 
    ...
    weakness: "Emptiness, happiness rises not as much as the others when exploring"
  },
];
...
function CharacterSelect() {
  ...
  useEffect(() => {
    ...
    const moveSound = new Audio(`${process.env.PUBLIC_URL}/Sounds/moveandselect.mp3`);
    const startSound = new Audio(`${process.env.PUBLIC_URL}/Sounds/confirm.mp3`);
  ...
  const handleStartGame = () => {
    if (playerName.trim()) {
      ...
      if (confirmSound) {
        ...
        setTimeout(() => {
          localStorage.setItem("selectedCharacter", characters[currentCharIndex].id);
  ...
  return (
    <div className="character-select-container" style={containerStyle}>
      ...
      <div className="character-selection">
        <div className="arrow" onClick={() => handleCharacterChange(-1)}>&#60;</div>
          ...
          <div className="character-container">
            {characters.map((char, index) => (
              <img
                ...
                className={`character ${index === currentCharIndex ? 'active' : ''}`}
                alt={char.name}
              />
            ))}
          </div>
